...
import {Box, Button, FormControlLabel, Checkbox, FormGroup} from '@mui/material';
...
export default function ChartToggle(props: ChartToggleProps): JSX.Element {
  ...
  const listfilter = () => {
    if (filterList && filterList.length >= 1) {
      ...
      return (
        <Box>
          <p>Es wurden keine Filter erstellt</p>
        </Box>
      );
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <FormGroup>{listfilter()}</FormGroup>
      </Box>
      <Box>
        <Button
          sx={{

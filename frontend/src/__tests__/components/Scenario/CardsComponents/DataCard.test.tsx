...
// Wrapper component to provide mock data to DataCard
function DataCardTest() {
  const mockIndex = 0;
  const mockCompartmentValues: Dictionary<number> = {
    'Compartment 1': 10,
    'Compartment 2': 20,
    'Compartment 3': 30,
  };
  const mockStartValues: Dictionary<number> = {
    'Compartment 1': 100,
    'Compartment 2': 200,
    'Compartment 3': 307,
  };
  const mockLabel = 'Scenario 1';
  const mockCompartmentsExpanded = true;
  const mockCompartments = ['Compartment 1', 'Compartment 2', 'Compartment 3'];
  const mockSelectedCompartment = 'Compartment 1';
  const mockSelectedScenario = true;
  const mockColor = 'primary';
  const mockActiveScenarios = [0, 1, 2];
  const mockFilterValues: Dictionary<filterValue[]> = {
    '0': [
  ...
  const mockMinCompartmentsRows = 1;
  const mockMaxCompartmentsRows = 3;
  const mockGroupFilters: Dictionary<GroupFilter> = {
    '0': {
      id: 'group1',
      name: 'Group 1',
      isVisible: true,
  ...
  const [activeScenarios, setActiveScenarios] = useState<number[] | null>(mockActiveScenarios);
  const [selectedScenario, setSelectedScenario] = useState<number | null>(mockIndex);

  return (
    <ThemeProvider theme={Theme}>
      <DataCard
        index={mockIndex}
        compartmentValues={mockCompartmentValues}
        startValues={mockStartValues}
        label={mockLabel}
        compartmentsExpanded={mockCompartmentsExpanded}
        compartments={mockCompartments}
        selectedCompartment={mockSelectedCompartment}
        selectedScenario={mockSelectedScenario}
        color={mockColor}
        activeScenarios={activeScenarios}
        filterValues={mockFilterValues}
        numberSelectedScenario={mockIndex}
        minCompartmentsRows={mockMinCompartmentsRows}
        maxCompartmentsRows={mockMaxCompartmentsRows}
        setSelectedScenario={setSelectedScenario}
        setActiveScenarios={setActiveScenarios}
        groupFilters={mockGroupFilters}
      />
    </ThemeProvider>
  );
}
...
describe('DataCard', () => {
  ...
  test('checks visibility based on group filters', () => {
    ...
    if (mockGroupFilters['0'].isVisible) {
      expect(group1).toBeInTheDocument();
    } else {
      expect(group1).not.toBeInTheDocument();
    }

    if (mockGroupFilters['1'].isVisible) {
